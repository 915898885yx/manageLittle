<template>
  <div class="studentImport" v-loading="fullLoading">
    <div class="search">
      <el-row class="row">
        <el-col :span="8">
          <label for="">项目：</label>
      ...
      <el-row class="row">
        ...
        <el-col :span="8">
          ...
          <el-select size="small" v-model="queryStudent.examineStatus" placeholder="请选择审核状态" class="inlineBlock">
            <el-option
        ...
        <el-col :span="8">
          ...
          <el-button size="small" type="primary" @click="downLoadById">下载</el-button>
    ...
    <div class="tableBox">
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            {{scope.row.examineStatus == '1' ? '审核通过': scope.row.examineStatus == '2'? '审核拒绝': ''}}
          </template>
        </el-table-column>
        <el-table-column
          label="操作">
...
<script>
  import {
    ...
    studentUploadImage,
    downloadDownloadStudentInfo,
    downloadDownloadStudentInfoByIds
  } from 'api/main'
  export default {
    data () {
      return {
    ...
    methods: {
      ...
      clearData () {
        ...
        this.queryStudent.examineStatus = ""
      },
      uploadImgs () {
        this.$nextTick(() => {
      ...
      async downLoadAll () {
        ...
        let res = await downloadDownloadStudentInfo(data)
      ...
      async downLoadById () {
        if (this.multipleSelection.length == 0) {
          this.$message.warning("请选择要下载得数据")
          return false
        }
        if (this.multipleSelection[0].examineStatus != '1') {
          this.$message.warning("只允许下载审核通过的数据")
          return false
        }
        const data = {
          ids: ""
        ...
        data.ids = ids.join(',')
        let res = await downloadDownloadStudentInfoByIds(data)
        let blob = new Blob([res], {type : 'application/x-zip-compressed '})
        let url = window.URL.createObjectURL(blob)
        let a = document.createElement('a')
        a.download = "学生信息.zip"
