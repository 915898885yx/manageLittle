<template>
  <div class="studentImport" v-loading="fullLoading">
    <div class="search">
      ...
      <el-row class="row">
        ...
        <!-- <el-col :span="8">
          ...
          <el-select v-model="queryStudent.examineStatus" placeholder="请选择审核状态" class="inlineBlock">
            <el-option
    ...
    <el-dialog
      ...
      <div class="detailtLine">
        <label for="">原始图片：</label>
        <img class="detailValue" :src="defailObj.oldImagePath" alt="">
      </div>
      <div class="detailtLine">
      ...
      <span slot="footer" class="dialog-footer">
        <el-button size="small" @click="checkOk('2')">拒 绝</el-button>
        <el-button size="small" type="primary" @click="checkOk('1')">通 过</el-button>
      </span>
    </el-dialog>
  </div>
</template>
<script>
  ...
  export default {
    ...
    methods: {
      ...
      checkOk (val) {
        const data = {
          id: this.defailObj.id,
          examineStatus: val
        }
        this.fullLoading = true
        studentSaveOrUpdateStudent(data).then(res => {
          if (res.code == 0) {
